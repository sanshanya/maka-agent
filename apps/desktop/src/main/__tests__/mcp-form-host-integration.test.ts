...
for (const action of ['accept', 'decline', 'cancel'] as const) {
  test(`Desktop modern MCP ${action} travels through the canonical Host form and back to the server`, { timeout: 15_000 }, async () => {
    await withFixture(async (manager, server, provider) => {
      await withClientCapabilityFormHost(provider, async (host) => {
        ...
        if (action === 'accept') {
          const rejected = await host.answer(pending.requestId, { action, values: { ...values, confirm: 'yes' } });
...
test('Desktop handles a second MCP question as a new canonical Host form', { timeout: 15_000 }, async () => {
  await withFixture(async (_manager, server, provider) => {
    ...
    server.respond = (params) => {
      if (params.inputResponses && ++answeredRounds === 1) return originalRespond({ name: params.name });
...
for (const exit of ['stop', 'disconnect', 'provider loss'] as const) {
  test(`Desktop ${exit} closes a pending modern MCP Host form without a user answer or retry`, { timeout: 15_000 }, async () => {
    await withFixture(async (manager, server, provider) => {
      await withClientCapabilityFormHost(provider, async (host) => {
        const result = host.start();
        const pending = await host.pending();
        ...
        assert.equal((await host.answer(pending.requestId, { action: 'accept', values })).ok, false);
        assert.equal(host.timers.size, 0);
      });
    });
  });
...
async function withFixture(run: (
  ...
  try {
    ...
    try { await run(manager, server, provider); } finally { await provider.close(); }
  } finally {
    await manager.close();
    await server.close();
  }
...
function createProvider(manager: McpClientManager) {
  return createDesktopNativeCapabilityProvider({
    ...
    releaseBrowserSession() {}, computerUseTools: [] as never,
    releaseDesktopInteractionSession() {},
    additionalGroups: () => [{
      offerId: 'desktop_mcp_fixture', label: 'MCP fixture', description: 'Modern form fixture',
      tools: buildMcpToolsWithIdentities(manager), dynamic: true,
    }],
  });
}
